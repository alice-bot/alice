...
__This is a work-in-progress branch of the next version of Alice. Lots of breaking
changes here. Probably not stable in the slightest.__

For an example bot, see the [A2ex] bot. For an example
handler, see [Google Images Handler].

[A2ex]: https://github.com/adamzaninovich/a2ex
[Google Images Handler]: https://github.com/alice-bot/alice_google_images

## Adapter Plugins

Alice 2 has an adapter system that allows you to connect your bot to several
different systems such as Slack and Discord (planned). Alice comes with some
basic adapters like the Console and Test adapters. Other adapters like the Slack
adapter are separate packages that can be included and configured in your bot.

To use the [Slack adapter], you'll also need a Slack API token which can be
retrieved from the [Web API page] or by creating a new [bot integration].

[Slack adapter]: https://github.com/adamzaninovich/alice_slack_adapter
...
_note: none of these currently work with alice2_
...
## Note:
_none of the following has been updated for alice2_
